...
static void player_level_update(){//每次碰到与经验相关操作都调用
    ...
    if(s_player->level<10) s_player->level_stage=0;
    else if(s_player->level<15) s_player->level_stage=1;
    else if(s_player->level<20) s_player->level_stage=2;
    else if(s_player->level<25) s_player->level_stage=3;
    else if(s_player->level<30) s_player->level_stage=4;
    else if(s_player->level<40) s_player->level_stage=5;
    else if(s_player->level>=40) s_player->level_stage=6;
};
...
bool player_plant(field_t *field,crop_type_t crop_type){
    if(s_player->seed_bag[crop_type]>0){
        if(field_plant(field,crop_type)){
            ...
            s_player->experience+=plant_exp_earn;
            player_level_update();
            return true;
        }
    }
...
bool player_harvest(field_t *field){
    if(field->stage==CROP_STAGE_READY){
        ...
        s_player->experience+=output*harvest_exp_earn[crop_type];
        player_level_update();
        return true;
    }
    return false;
...
bool player_sold(crop_type_t crop_type,int n){
    if(s_player->harvest_bag[crop_type]>=n){
        s_player->coins+=n*harvest_price[crop_type];
...
bool player_buy_farm_size_update(){
    ...
    if(farm->size_level>=3) return false;
...
int harvest_exp_earn[CROP_TYPE_NONE]={
    2,
    3,
    4
};
