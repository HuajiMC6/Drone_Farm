...
static void stage_update(field_t* field){//也就1刻的事儿，不管1e-6了
    ...
    if(percent<0.1) field->stage=CROP_STAGE_SEED;
    else if(percent<0.3) field->stage=CROP_STAGE_YOUNG;
    else if(percent<0.7) field->stage=CROP_STAGE_GROW;
    else if(percent<0.9) field->stage=CROP_STAGE_BLOOM;
    else if(percent<1.0) field->stage=CROP_STAGE_RIPE;
    else field->stage=CROP_STAGE_READY;
}
...
int field_harvest(field_t* field){
    if(field->crop_type==CROP_TYPE_NONE||field->stage!=CROP_STAGE_READY) return 0;
