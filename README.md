...
无人机是游戏中最具策略性的玩法，拥有三种工作状态：

| 状态 | 触发方式 | 行为 |
|:---:|:---|:---|
| 🟢 **空闲** | 默认 / 任务完成 | 停留在农田上方待命 |
| 🔵 **侦查** | 切换至侦查模式 | 摇杆手动飞行，飞过田块时自动检测虫害类型，HUD 实时反馈 |
| 🟠 **喷洒** | 切换至喷洒模式 | 按算法规划的路径自动飞行，对已侦查田块逐一喷洒匹配农药 |

**路径规划**：采用 **贪心 + 2-opt 局部优化** 算法 —— 先以最近邻贪心构造初始路径，再通过 2-opt 边交换迭代缩短总行程。

**作业机制**：

- 无人机拥有独立**药仓**（容量可升级），执行喷洒前需从背包装载对应农药
- 喷洒时自动匹配虫害类型，药量不足则无法喷洒，需返回后手动补药
- 侦查结果（各虫害数量）与剩余药量通过 **HUD 面板实时显示**
- 同一田块可**多次感染虫害**，每次均需重新侦查 → 喷洒
...
游戏提供 **6 大升级维度**，构建丰富策略空间：

| 类别 | 升级项 | 等级范围 | 价格梯度 | 效果 | 生效范围 |
|:---:|:---|:---:|:---|:---|:---:|
| 🚁 无人机 | 飞行速度 | 0→3 | 1000/2500/5000 | 速度 10→15→20→30 | 全局 |
| 🚁 无人机 | 药仓容量 | 0→3 | 1000/2500/5000 | 容量 10→20→30→50 | 全局 |
| 🏞️ 农场 | 规模 | 0→3 | 5000/10000/20000 | 4×4→5×5→6×6→7×7 | 全局 |
| 🌿 田块 | 产量 | 0→3 | 2500/5000/10000 | 产量 +0%→+20%→+40%→+50% | 单块 |
| ⏱️ 田块 | 生长速度 | 0→3 | 2500/5000/10000 | 时间 ×1.0→×0.9→×0.8→×0.7 | 单块 |
| 🛡️ 田块 | 耐虫性 | 0→3 | 10000/20000/50000 | 抗性 +0→+0.1→+0.25→+0.4 | 单块 |
...
| 📡 **Event** | `modules/event.c/.h` | 环形事件队列：17 种事件类型，解耦逻辑层与 UI 层通信 |
...
| 📈 **二次损伤模型** | 基于二次函数 $a(t-b)^2+c$ 的虫害概率曲线，不同虫害对应不同 $(a,b,c)$ 参数，实现分阶段差异化风险 |
...
| 成员 | 姓名 | 学号 | 主要分工 | 代码量 |
|:---:|:---|:---|:---|:---:|
| I | **刘俊辉** | U202514535 | UI 交互系统设计与实现（主界面/商店/仓库/设置/无人机/升级等全部页面布局与回调）、3 个组件库封装、硬件驱动与 UI 的对接（摇杆/扬声器） | ~4043 行 |
| II | **吴天宇** | U202514546 | 核心业务逻辑模块设计与实现（farm/field/player/drone/event/data）、主循环调度、事件系统设计、存档系统实现、游戏配置数据设计、系统集成测试、图片与音效资源搜集 | ~1314 行 |

> 🏫 人工智能 2503 班 · C 语言课程设计
...
| 核心逻辑开发 | farm/field/player/drone/event/data 模块实现 | ✅ 已完成 |
| UI 主流程开发 | 主界面、农田网格、窗口交互、事件刷新 | ✅ 已完成 |
| 联调 & 稳定性修复 | 功能联调、边界错误修复、内存稳定性验证 | ✅ 已完成 |
| 收尾 & 验收 | 体验优化、数据持久化完善、文档与答辩 | ✅ 已完成 |
...
| 类别 | 代码行数 | 占比 |
|:---|:---:|:---:|
| 核心业务逻辑模块（modules/） | 1,245 行 | 23.2% |
| UI 布局实现（ui/layouts/） | 2,410 行 | 45.0% |
| UI 回调函数（ui/callbacks/） | 615 行 | 11.5% |
| UI 控件库（ui/widgets/） | 635 行 | 11.9% |
| UI 主模块与公共工具（ui/） | 383 行 | 7.1% |
